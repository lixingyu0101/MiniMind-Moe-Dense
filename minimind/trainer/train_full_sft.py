...
def validate(epoch, loader, model, args, wandb=None):
    model.eval()
    loss_fct = nn.CrossEntropyLoss(reduction='none')
    ...
    print(f"[VAL] Epoch [{epoch+1}] loss: {val_loss:.6f}, ppl: {val_ppl:.2f}")
    if wandb: wandb.log({"val_loss": val_loss, "val_ppl": val_ppl})
    model.train()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--val_data_path", type=str, default=None, help="验证数据路径，可选")
    ...
    if args.val_data_path:
        val_ds = SFTDataset(args.val_data_path, tokenizer, max_length=args.max_seq_len)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if val_loader is not None and is_main_process():
            validate(epoch, val_loader, model, args, wandb)
